...
python scripts\stage5_pv_v2_runner.py --dataset dfire --family yolo26n --seed 33 --mode all --arm hardneg_sched --epochs 1 --batch 16 --workers 2 --device 0 --val false --limit 256 --run-root runs\detect\runs_stage5_pv_v2_mincheck --out-root formal_results\stage5_pv_v2_mincheck --overwrite
if %ERRORLEVEL% NEQ 0 exit /b %ERRORLEVEL%

echo [ok] H02 YOLO26n seed33 hardneg_sched mincheck completed.
endlocal
